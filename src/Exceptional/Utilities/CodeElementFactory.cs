namespace ReSharper.Exceptional.Utilities
{
    ...
    public sealed class CodeElementFactory
    {
        #region member vars
        ...
        /// <summary>
        /// Creates a specific catch clause with given <paramref name="exceptionType" /> and <paramref name="catchBody" />
        /// .
        /// </summary>
        ...
        public ISpecificCatchClause CreateSpecificCatchClause(IDeclaredType exceptionType, IBlock catchBody, string variableName)
        {
            ...
            if (catchBody == null)
            {
                catchBody = _factory.CreateBlock("{$1    // TODO: Handle the $0$1}", exceptionType.GetClrName().FullName, Environment.NewLine);
            }
            {
                var exceptionDeclaration = catchClause.ExceptionDeclaration;
                if (exceptionDeclaration == null)
